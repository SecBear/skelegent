#![deny(missing_docs)]
//! OpenAI Codex (Responses API) provider for neuron-turn.
//!
...
//!
//! ```ignore
//! use neuron_provider_codex::CodexProvider;
//!
                        ...
                        if let Some(item) = event.data.get("item") {
                            ...
                            match item_type {
                                ...
                                "function_call" => {
                                    ...
                                    // Compose the neuron tool ID as "call_id|item_id"
                                    let neuron_id = format!("{call_id}|{}", current_tool_item_id);
