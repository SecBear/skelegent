...
/// TODO: Evaluate migration to tagged serde matching ProtoJSON discriminated unions.
/// Changing this would fix the `Data` variant's "match anything" footgun but break
/// existing wire format. Only safe if all consumers and tests are updated together.
///
