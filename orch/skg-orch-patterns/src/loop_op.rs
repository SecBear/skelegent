...
//! (returning [`Outcome::Limited { limit: LimitReason::MaxTurns }`] in the latter case).
...
use layer0::operator::{
    LimitReason, Operator, OperatorInput, OperatorOutput, Outcome, TerminalOutcome, TriggerType,
};
...
pub struct LoopOperator {
    ...
    /// `Outcome::Limited { limit: LimitReason::MaxTurns }`.
    max_iterations: u32,
...
impl LoopOperator {
    ...
    /// `Outcome::Limited { limit: LimitReason::MaxTurns }` if the predicate never fires.
    pub fn new(
        body: OperatorId,
        dispatcher: Arc<dyn Dispatcher>,
        max_iterations: u32,
