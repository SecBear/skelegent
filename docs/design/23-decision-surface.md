...
| L5 | Observability | Middleware traits and `Effect::Log` | Context event stream on `Context` | `ContextObserver`, `ExecutionTrace`, middleware selection, durable audit subscribers | `orch/skg-orch-kit/src/{observe.rs,runner.rs}`, middleware crates, context stream | Implemented. Enough surface exists to observe worker execution without Layer 0 expansion. |
