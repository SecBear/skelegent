...
`Environment::run()` is called by **orchestrator implementations** (Layer 2), not by operators or composition code directly. Operators receive dispatch capability via `Arc<dyn Dispatcher>` injected at construction time; the orchestrator mediates the environment boundary transparently. `Dispatcher` remains the immediate invocation contract. Durable lifecycle/control belongs to orchestration code above Layer 0 rather than to the Layer 0 protocol itself.
